...
OBJS=mime.o ftp.o http.o smb.o common.o connstat.o httpstream.o dirfd.o
HTTPS_OBJS=mime.o ftp.c http.c smb.c common.c connstat.o httpstream.o dirfd.o
PHTTPS_OBJS=mime.o ftp.o http.c smb.c common.c connstat.o httpstream.o dirfd.o

$(OBJS) https.o gatling.o: gatling.h version.h gatling_features.h dirfd.h

tlsgatling: gatling.c ssl.o $(HTTPS_OBJS) libsocket libiconv libcrypt libsslpthread
	$(CC) -o $@ gatling.c ssl.o $(HTTPS_OBJS) $(CFLAGS) -DSUPPORT_HTTPS $(LDFLAGS) -lssl -lcrypto $(LDLIBS) `cat libsslpthread`

tlsgatling_nofail: gatling.c ssl.o $(HTTPS_OBJS) libsocket libiconv libcrypt libsslpthread
	-$(CC) -o tlsgatling gatling.c ssl.o $(HTTPS_OBJS) $(CFLAGS) -DSUPPORT_HTTPS $(LDFLAGS) -lssl -lcrypto $(LDLIBS) `cat libsslpthread`

ptlsgatling: gatling.c pssl.o $(PHTTPS_OBJS) libsocket libiconv libcrypt
	$(CC) -o $@ gatling.c pssl.c $(PHTTPS_OBJS) $(CFLAGS) -DSUPPORT_HTTPS -DUSE_POLARSSL $(LDFLAGS) -lmbedtls $(LDLIBS)

ptlsgatling_nofail: gatling.c pssl.o $(PHTTPS_OBJS) libsocket libiconv libcrypt
	-$(CC) -o ptlsgatling gatling.c pssl.c $(PHTTPS_OBJS) $(CFLAGS) -DSUPPORT_HTTPS -DUSE_POLARSSL $(LDFLAGS) -lmbedtls $(LDLIBS)
...
libsslpthread: ssl.c
	if $(CC) $(CFLAGS) -o tryssl ssl.c -DMAIN -lowfat -lssl -lcrypto >/dev/null 2>&1 ; then echo ""; else \
	if $(CC) $(CFLAGS) -o tryssl ssl.c -DMAIN -lowfat -lssl -lcrypto -lpthread >/dev/null 2>&1 ; then echo "-lpthread"; \
	fi; fi > $@
	rm -f tryssl
...
clean:
	rm -f $(ALLTARGETS) *.o version.h core *.core libsocket libsocketkludge.a dummy.c libiconv libcrypt havesetresuid.h md5lib havealloca.h tlsgatling ptlsgatling libsslpthread
