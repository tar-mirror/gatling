...
static int ftp_open(struct http_data* h,const char* s,int forreading,int sock,const char* what,struct stat* ss) {
  ...
  *y=0;
  ...
  if (x[1]) {
    switch (forreading) {
    case 1: open_for_reading(&fd,x+1,ss,dirfd); break;
    case 0: open_for_writing(&fd,x+1,dirfd); break;
    case 2: fd=mkdirat(dirfd,x+1,0777);
	    if (!fd) fchmodat(dirfd,x+1,0777,0);
	    break;
    }
  }
#ifdef DEBUG
...
#endif
  int dir2fd=-1;
  if (!x[1] || (dir2fd=openat(dirfd,x+1,O_RDONLY|O_PATH|O_DIRECTORY|O_CLOEXEC))!=-1) {		/* it's a directory */
    pathprefix="";
    match=0;
  } else {
    if (s[0]!='/') {	/* foo/$fnord */
	...
	z=str_rchr(x,'/');
	x[z]=0;
	if (x[0]=='/' && x[1] && (dir2fd=openat(dirfd,x+1,O_RDONLY|O_PATH|O_DIRECTORY|O_CLOEXEC))==-1) {
notfound:
	...
	match=x+z+1;
      ...
      int z=str_rchr(x,'/');
      x[z]=0;
      if (x[0]=='/' && x[1] && (dir2fd=openat(dirfd,x+1,O_RDONLY|O_PATH|O_DIRECTORY|O_CLOEXEC))==-1) goto notfound;
      match=x+z+1;
      pathprefix=alloca(z+2);
...
static int ftp_cwd(struct http_data* h,char* s) {
  ...
  *y=0;
  ...
  if (x[1] && (dir2fd=openat(dirfd,x+1,O_RDONLY|O_PATH|O_DIRECTORY|O_CLOEXEC))==-1) {
...
syntaxerror:
	...
	byte_copy(ip,12,V4mappedprefix);
      ...
      c+=r+1;
      port=l<<8;
      r=scan_ulong(c,&l); if (!r || l>255) goto syntaxerror;
      port+=l;
    }
