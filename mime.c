...
enum charset sniffencoding(const char* buf,unsigned int len) {
  int i;
  /* see if it looks like plain text */
  for (i=0; i<len; ++i) {
    unsigned char c = (unsigned char)buf[i];
    if (c<' ' && c!='\n' && c!='\t' && c!='\r')
      return BINARY;
  }
  /* if we get this far, it's probably plain text; check for utf-8 */
  for (i=0; i<len; ) {
    size_t k;
    if ((k=scan_utf8(buf+i,len-i,NULL))>1)
      return UTF8;
    else if (k==0)
      return LATIN1;
    else
...
const char* mimetype(const char* filename,int fd) {
...
#ifdef SUPPORT_MIMEMAGIC
  {
	...
	return "audio/x-wav";
      ...
      switch (sniffencoding(buf,r)) {
      case ASCII: return "text/plain";
      case LATIN1: return "text/plain; charset=\"iso-8859-1\"";
      case UTF8: return "text/plain; charset=\"utf-8\"";
