...
void catencoded(array* a,const char* s) {
...
void cathtml(array* a,const char* s) {
  unsigned int len=str_len(s);
  if (len) {
...
void catxmlencoded(array* a,const char* s) {
  unsigned int len=str_len(s);
  if (len) {
    char* buf=alloca(fmt_xml(0,s,len));
    array_catb(a,buf,fmt_xml(buf,s,len));
  }
}

void cathtmlutf8(array* a,const char* s) {
...
static int http_dirlisting(struct http_data* h,DIR* D,const char* path,const char* arg,enum Method method,int dirfd) {
  long i,o,n;
  struct dirent* d;
  int (*sortfun)(de*,de*);
#ifdef SUPPORT_DAV
  ...
  o=n=0;
#ifdef SUPPORT_DAV
  depth=http_header(h,"Depth");
  if (method==PROPFIND) {
    de* x=array_allocate(&a,sizeof(de),n);
    if (x) {
      x->name=0;
      fstatat(dirfd,".",&x->ss,AT_SYMLINK_NOFOLLOW);	/* if this fails, we just serve bogus data */
      array_cats0(&b,".");
      ++n;
      o=2;
    }
  }
  if (method!=PROPFIND || (depth && *depth=='1'))
#endif
  while ((d=readdir(D))) {
#ifdef SUPPORT_DAV
    ...
    if (!x) break;
    x->name=o;
    if (fstatat(dirfd,d->d_name,&x->ss,AT_SYMLINK_NOFOLLOW)==-1) continue;
    if (S_ISLNK(x->ss.st_mode)) {
      struct stat tmp;
      if (fstatat(dirfd,d->d_name,&tmp,0)==0)
  ...
  if (method==PROPFIND) {
    ...
    size_t i,j;
    if (host) {
      for (i=0; isalnum(host[i]) || host[i]=='-' || host[i]=='.' || host[i]==':'; ++i) ;
    } else
      i=FMT_IP6+sizeof(":65535");
    baseurl=alloca(i+sizeof("https://")+strlen(path)+2);
#ifdef SUPPORT_HTTPS
    if (h->t==HTTPSREQUEST)
      j=fmt_str(baseurl,"https://");
    else
#endif
      j=fmt_str(baseurl,"http://");
    if (host)
      j+=fmt_copybytes(baseurl+j,host,i);
    else {
      j+=fmt_ip6c(baseurl+j,h->myip);
      baseurl[j++]=':';
      j+=fmt_ulong(baseurl+j,h->myport);
    }
    j+=fmt_str(baseurl+j,path);
    baseurl[j]=0;
    array_cats(&c,"<?xml version=\"1.0\" encoding=\"utf-8\"?><D:multistatus xmlns:D=\"DAV:\">");
  } else {
#endif
  ...
  for (i=0; i<n; ++i) {
    ...
    if (!(ab[i].ss.st_mode & S_IROTH)) continue; /* skip files that are not world readable */
    if (name[0]==':') name[0]='.';

#ifdef SUPPORT_DAV
    if (method==PROPFIND) {
      array_cats(&c,"<D:response><D:href>");
      catxmlencoded(&c,baseurl);
      if (name[0]!='.' || name[1])
	catxmlencoded(&c,name);
      array_cats(&c,"</D:href><D:propstat><D:status>HTTP/1.1 200 OK</D:status><D:prop>");
      if (S_ISDIR(ab[i].ss.st_mode))
	array_cats(&c,"<D:getcontenttype/>");
      else {
	array_cats(&c,"<D:getcontenttype>");
	array_cats(&c,mimetype(name,-1));
	array_cats(&c,"</D:getcontenttype>");
      }
      array_cats(&c,"<D:getlastmodified>");
      buf[fmt_httpdate(buf,ab[i].ss.st_mtime)]=0;
      array_cats(&c,buf);
      array_cats(&c,"</D:getlastmodified><D:lockdiscovery/><D:ishidden>0</D:ishidden><D:supportedlock><D:lockentry><D:lockscope><D:exclusive/></D:lockscope><D:locktype><D:write/></D:locktype></D:lockentry><D:lockentry><D:lockscope><D:shared/></D:lockscope><D:locktype><D:write/></D:locktype></D:lockentry></D:supportedlock>");
      if (S_ISDIR(ab[i].ss.st_mode))
	array_cats(&c,"<D:getetag/><D:displayname>");
      else {
	array_cats(&c,"<D:getetag>\"");
	buf[j=fmt_ulong(buf,ab[i].ss.st_dev)]='.';
	array_catb(&c,buf,j+1);
	j=fmt_ulong(buf,ab[i].ss.st_ino);
	array_catb(&c,buf,j);
	array_cats(&c,"\"</D:getetag><D:displayname>");
      }
      catxmlencoded(&c,name);
      array_cats(&c,"</D:displayname><D:getcontentlanguage/><D:getcontentlength>");
      buf[fmt_ulonglong(buf,ab[i].ss.st_size)]=0;
      array_cats(&c,buf);
      array_cats(&c,S_ISDIR(ab[i].ss.st_mode) ?
	"</D:getcontentlength><D:iscollection>1</D:iscollection><D:creationdate>" :
	"</D:getcontentlength><D:iscollection>0</D:iscollection><D:creationdate>" );
      buf[fmt_iso8601(buf,ab[i].ss.st_ctime)]=0;
      array_cats(&c,buf);
      array_cats(&c,S_ISDIR(ab[i].ss.st_mode) ?
	"</D:creationdate><D:resourcetype><D:collection/></D:resourcetype></D:prop></D:propstat></D:response>" :
	"</D:creationdate><D:resourcetype/></D:prop></D:propstat></D:response>" );
    } else {
#endif
...
static int proxy_connection(int sockfd,char* c,const char* dir,struct http_data* ctx_for_sockfd,int isexec,const char* args,struct dircacheentry* de,int dirfd) {
  /* c is the filename
  ...
  if (de && de->proxy==-1) return -3;
  if (de && de->proxy!=1) {
    if (!isexec && fstatat(de->fd,".proxy",&ss,0)==-1) {
      if (de) de->proxy=-1;
      return -3;
    }
  }
  ...
  while (x) {
	...
	/* for SCGI and FASTCGI we expect the file to exist (if not empty) */
	if (*d!=0 && (x->proxyproto == SCGI || x->proxyproto == FASTCGI)) {
	  struct stat ss;
	  /* does the file actually exist? */
	  if (fstatat(dirfd,d,&ss,AT_SYMLINK_NOFOLLOW)) {
	    if (errno==ENOTDIR) {	/* we have PATH_INFO */
	      char save=c[matches.rm_eo];
	      int r;
	      c[matches.rm_eo]=0;
	      r=fstatat(dirfd,d,&ss,AT_SYMLINK_NOFOLLOW);
...
freeandfail:
	...
	case 'P':
		  ...
		  break;
#ifdef SUPPORT_DAV
	case 'O': method=(ctx_for_sockfd->t==HTTPREQUEST)?"OPTIONS":"OPTIONS/SSL"; break;
//	case 'L': method=(ctx_for_sockfd->t==HTTPREQUEST)?"LOCK":"LOCK/SSL"; break;
//	case 'U': method=(ctx_for_sockfd->t==HTTPREQUEST)?"UNLOCK":"UNLOCK/SSL"; break;
#endif
	}
#else
	switch (*tmp) {
	...
	case 'P': method=(tmp[1]=='O')?"POST":
		  ...
		  break;
#ifdef SUPPORT_DAV
	case 'O': method="OPTIONS"; break;
//	case 'L': method="LOCK"; break;
//	case 'U': method="UNLOCK"; break;
#endif
	}
#endif
...
#elif defined(USE_POLARSSL)
	      mbedtls_ssl_write(&ctx_for_sockfd->ssldata->ssl,(const unsigned char*)contmsg,sizeof(contmsg)-1);
#else
...
#elif defined(USE_POLARSSL)
    i=mbedtls_ssl_read(&H->ssldata->ssl,(unsigned char*)buf,l);
    if (i<0) {
...
#elif defined(USE_POLARSSL)
	if (i==MBEDTLS_ERR_SSL_WANT_READ) io_eagain_read(sockfd);
	else if (i==MBEDTLS_ERR_SSL_WANT_WRITE) io_eagain_write(sockfd);
#else
...
int http_dohtaccess(struct http_data* h,const char* filename,int nobody,int dirfd,struct dircacheentry* de) {
  size_t filesize;
  const char* map;
  const char* s;
  char* auth;
  ...
  int r=0;
  if (de) {
    if (de->htaccess_global_len) {
      if (!de->htaccess_global) return 1;
      map=de->htaccess_global;
      filesize=de->htaccess_global_len;
    } else {
      map=mmap_readat(filename,&filesize,dirfd);
      de->htaccess_global=map;
      de->htaccess_global_len=map?filesize:1;
    }
  } else
    map=mmap_readat(filename,&filesize,dirfd);
  if (!map) return 1;
...
void do_dir_redirect(struct http_data* h,const char* filename,int64 s,enum Method method) {
  char* nh;
  int i;
  char* host=http_header(h,"Host");
#ifdef SUPPORT_HTTPS
  ...
  size_t hl;
  if (!host) {
    /* This should not happen. Someone is sending us a GET for a
     * directory but no Host header. Without a host header, we cannot
     * construct a redirect URL to send him to, so we will just 404. */
    httperror(h,"404 Not Found","No such file or directory.",method==HEAD);
    h->hdrbuf=h->bodybuf=0;	/* httperror already calls iob_addbuf_free, so we need to make sure the caller doesn't call it again */
    return;
  }
  hl=str_chr(host,'\n');
  ...
  iob_addbuf_free(&h->iob,nh,i);
}
#endif

int word(const char* s,const char* end,const char* theword,size_t n) {
  return (end-s>=n && !memcmp(s,theword,n) &&
	  (s[n]==',' || s[n]==' ' || s[n]=='\r' || s[n]=='\n'));
}

int64 http_openfile(struct http_data* h,char* filename,struct stat* ss,int sockfd,enum Method method) {
#ifdef SUPPORT_PROXY
  int noproxy=0;
#endif
  char* dir=0;
...
#endif
  {
    ...
    if (tmp) {	/* yeah this is crude, but it gets the job done */
      ...
      const char* cur;
      for (cur=tmp; cur<end; ) {
	if (word(cur,end,"gzip",4))
	  doesgzip=1;
#ifdef SUPPORT_BZIP2
	else if (word(cur,end,"bzip2",5))
	  doesbzip2=1;
#endif
#ifdef SUPPORT_BROTLI
	else if (word(cur,end,"br",2))
	  doesbrotli=1;
#endif
...
makefakeheader:
  ...
  int dirfd=AT_FDCWD;
  struct dircacheentry* de=0;
  if (virtual_hosts>=0)
    if ((dirfd=getdirfd2(dir=s,now.sec.x-4611686018427387914ULL,&de))==-1)
      if ((dirfd=getdirfd2(dir="default",now.sec.x-4611686018427387914ULL,&de))==-1) {
	if (virtual_hosts==1) {
...
#ifdef SUPPORT_HTACCESS
  if (http_dohtaccess(h,".htaccess_global",method==HEAD,dirfd,de)==0) return -5;
#endif
  ...
  {
    int res;
    switch ((res=proxy_connection(sockfd,Filename,dir,h,0,args,de,dirfd))) {
  ...
  if (Filename[(i=str_len(Filename))-1] == '/') {
    ...
    if (defaultindex) {
      ...
      if (fstatat(dirfd,Filename+1,ss,0)==0) {
...
#define O_CLOEXEC 0
#endif
    int dir2fd=dirfd;
    if (Filename[1] && (dir2fd=openat(dirfd,Filename+1,O_RDONLY|O_PATH|O_DIRECTORY|O_CLOEXEC))==-1) return -1;
#ifdef SUPPORT_HTACCESS
    if (http_dohtaccess(h,".htaccess",method==HEAD,dir2fd,0)==0) {
      if (dir2fd!=-1) close(dir2fd);
      return -5;
    }
#endif
	...
	method==PROPFIND ||
#endif
	!open_for_reading(&fd,"index.html",ss,dir2fd)) {
      DIR* d;
      if (errno==ENOENT)
	if (http_redirect(h,"index.html")) {
	  if (dir2fd!=-1) close(dir2fd);
	  return -4;
	}
      if (!directory_index) {
	...
	return -1;
      ...
      if (!(d=fdopendir(dir2fd))) {
	if (dir2fd!=-1) close(dir2fd);
	return -1;
      }
      if (!http_dirlisting(h,d,Filename,args,method,dir2fd)) return -1;
#ifdef USE_ZLIB
      if (doesgzip) {
	...
	if (byte_equal(temp,2,"#!") || byte_equal(temp,4,"\177ELF")) {
	  ...
	  temp[i]=0;
	  switch ((res=proxy_connection(sockfd,temp,dir,h,1,args,de,dirfd))) {
	  case -2: break;
	  case -1: return -1;
	  default:
	    if (res>=0) {
    ...
    if (doesbrotli) {
      int64 gfd;
      if (open_for_reading(&gfd,"index.html.br",ss,dir2fd)) {
	io_close(fd);
	fd=gfd;
	h->encoding=BROTLI;
      }
    ...
    if (doesbzip2) {
      int64 gfd;
      if (open_for_reading(&gfd,"index.html.bz2",ss,dir2fd)) {
	io_close(fd);
	fd=gfd;
	h->encoding=BZIP2;
      }
    ...
    if (doesgzip) {
      int64 gfd;
      if (open_for_reading(&gfd,"index.html.gz",ss,dir2fd)) {
	io_close(fd);
	fd=gfd;
	h->encoding=GZIP;
      }
...
itsafile:
	...
	if (http_dohtaccess(h,x,method==HEAD,dirfd,0)==0) return -5;
      } else
	if (http_dohtaccess(h,".htaccess",method==HEAD,dirfd,0)==0) return -5;
    }
#endif

    /* For /test/t.cgi/fnord open_for_reading fails with ENOTDIR. */
    if (!open_for_reading(&fd,Filename+1,ss,dirfd)) {
      if (errno==ENOENT)
	...
	for (i=0; fn[i]; ++i) {
	  if (fn[i]=='/') {
	    ...
	    if (!S_ISDIR(ss->st_mode)) {	/* found first non-dir entry, hopefully our CGI */
	      if (!(ss->st_mode&S_IROTH) || (fd=openat(dirfd,fn,O_RDONLY)==-1))
		return -1;
    ...
    if (S_ISDIR(ss->st_mode)) {
       ...
       * redirect */
      do_dir_redirect(h,Filename,sockfd,method);
      return -4;
    }
#endif
...
foundcgi:
	...
	int res;
	switch ((res=proxy_connection(sockfd,Filename,dir,h,1,args,de,dirfd))) {
	case -2: break;
	case -1: return -1;
	default:
	  if (res>=0) {
...
#ifdef SUPPORT_BROTLI
      if (doesbrotli) {
	Filename[i+fmt_str(Filename+i,".br")]=0;
	if (open_for_reading(&gfd,Filename+1,ss,dirfd)) {
	  io_close(fd);
	  fd=gfd;
	  h->encoding=BROTLI;
	}
...
static int header_diff(const char* s,const char* t) {
  ...
  register int j;
  for (;;) {
    if ((j=(mytolower(*s)-mytolower(*t)))) break;
    if (!*t) break;
    ++s; ++t;
  }
  if (*s=='\r' || *s=='\n') j=-*t;
...
void httpresponse(struct http_data* h,int64 s,long headerlen) {
  ...
  c=array_start(&h->r);
  if (byte_equal(c,5,"GET /")) { method=GET; c+=4; }
  else if (byte_equal(c,6,"POST /")) { method=POST; c+=5; }
#ifdef SUPPORT_PUT
  else if (byte_equal(c,5,"PUT /")) { method=PUT; c+=4; }
#endif
#ifdef SUPPORT_DAV
  else if (byte_equal(c,10,"PROPFIND /")) { method=PROPFIND; c+=9; }
  else if (byte_equal(c,9,"OPTIONS /")) { method=OPTIONS; c+=8; }
//  else if (byte_equal(c,8,"UNLOCK /")) { method=UNLOCK; c+=7; }
//  else if (byte_equal(c,6,"LOCK /")) { method=LOCK; c+=5; }
#endif
  else if (byte_equal(c,6,"HEAD /")) { method=HEAD; c+=5; }
  else {
e400:
  ...
  }
  {
    ...
    *d=0;
    ...
    if (method==OPTIONS) {
      ...
      c+=fmt_strm(c,"HTTP/1.1 200 OK\r\n"
		  "Server: " RELEASE "\r\n",
		  (ss.st_mode & S_IWOTH) ?
		  "Allow: OPTIONS, GET, HEAD, PUT, POST, PROPFIND\r\n"
		  "Public: OPTIONS, GET, HEAD, PUT, POST, PROPFIND\r\n" :
		  "Allow: OPTIONS, GET, HEAD, POST, PROPFIND, LOCK, UNLOCK\r\n"
		  "Public: OPTIONS, GET, HEAD, POST, PROPFIND, LOCK, UNLOCK\r\n",
		  "DAV: 1,2,3\r\n"
		  "Date: ");
      c+=fmt_httpdate(c,now.sec.x-4611686018427387914ULL);
      c+=fmt_str(c,"\r\nContent-Length: 0\r\n\r\n");

      iob_addbuf_free(&h->iob,nh,c-nh);

      goto fini;
    }
#endif
    fd=http_openfile(h,c,&ss,s,method);
    if (fd==-1) {
e404:
#ifdef SUPPORT_FALLBACK_REDIR
      if (redir)
	...
	if (h->t == HTTPSREQUEST)
	  buffer_puts(buffer_1,"HTTPS/");
#endif
	buffer_putm(buffer_1,methods[method],"/404 ");
	buffer_putulong(buffer_1,s);
	buffer_puts(buffer_1," ");
	buffer_putlogstr(buffer_1,c);
	buffer_puts(buffer_1," 0 ");
	...
	if (!c)
	  httperror(h,"500 Sorry","Out of Memory.",method==HEAD);
	else {
	  ...
	  if (logging) {
	    ...
	    if (h->t == HTTPSREQUEST)
	      buffer_puts(buffer_1,"HTTPS/");
#endif
	    buffer_putm(buffer_1,methods[method]," ");
	    buffer_putulong(buffer_1,s);
	    buffer_puts(buffer_1," ");
...
#ifdef SUPPORT_DAV
	  if (method==PROPFIND)
	    c+=fmt_str(c,"HTTP/1.1 207 Multi Status\r\nContent-Type: text/xml\r\nConnection: ");
	  else
#endif
	  ...
	  if (h->encoding!=NORMAL) {
	    c+=fmt_str(c,"\r\nContent-Encoding: ");
#if defined(SUPPORT_BROTLI)
	    if (h->encoding==BROTLI)
	      c+=fmt_str(c,"br");
	    else
#endif
#if defined(SUPPORT_BZIP2)
	    if (h->encoding==BZIP2)
	      c+=fmt_str(c,"bzip2");
	    else
#endif
	...
	if (method==PROPFIND) {
	  ...
	  if (host) {
	    ...
	    i=FMT_IP6+sizeof(":65535");
	  memset(&x,0,sizeof(x));
	  baseurl=alloca(i+sizeof("https://")+strlen(c)+2);
#ifdef SUPPORT_HTTPS
	  if (h->t==HTTPSREQUEST)
	    j=fmt_str(baseurl,"https://");
	  else
	  ...
	  j+=fmt_str(baseurl+j,c);
	  baseurl[j]=0;
	  array_cats(&x,"<?xml version=\"1.0\" encoding=\"utf-8\"?><D:multistatus xmlns:D=\"DAV:\"><D:response><D:href>");
	  catxmlencoded(&x,baseurl);

	  array_cats(&x,"</D:href><D:propstat><D:status>HTTP/1.1 200 OK</D:status><D:prop>");
	  if (S_ISDIR(ss.st_mode))
	    array_cats(&x,"<D:getcontenttype/>");
	  else {
	    array_cats(&x,"<D:getcontenttype>");
	    array_cats(&x,mimetype(c,-1));
	    array_cats(&x,"</D:getcontenttype>");
	  }
	  array_cats(&x,"<D:getlastmodified>");

	  buf[fmt_httpdate(buf,ss.st_mtime)]=0;
	  array_cats(&x,buf);
	  array_cats(&x,"</D:getlastmodified><D:lockdiscovery/><D:ishidden>0</D:ishidden><D:supportedlock><D:lockentry><D:lockscope><D:exclusive/></D:lockscope><D:locktype><D:write/></D:locktype></D:lockentry><D:lockentry><D:lockscope><D:shared/></D:lockscope><D:locktype><D:write/></D:locktype></D:lockentry></D:supportedlock>");

	  if (S_ISDIR(ss.st_mode))
	    array_cats(&x,"<D:getetag/><D:displayname>");
	  else {
	    array_cats(&x,"<D:getetag>\"");
	    buf[j=fmt_ulong(buf,ss.st_dev)]='.';
	    array_catb(&x,buf,j+1);
	    j=fmt_ulong(buf,ss.st_ino);
	    array_catb(&x,buf,j);
	    array_cats(&x,"\"</D:getetag><D:displayname>");
	  }
	  {
	    const char* name=strrchr(baseurl,'/');
	    catxmlencoded(&x,name+1);
	  }
	  array_cats(&x,"</D:displayname><D:getcontentlanguage/><D:getcontentlength>");
	  buf[fmt_ulonglong(buf,ss.st_size)]=0;
	  array_cats(&x,buf);

	  array_cats(&x,S_ISDIR(ss.st_mode) ?
	    "</D:getcontentlength><D:iscollection>1</D:iscollection><D:creationdate>" :
	    "</D:getcontentlength><D:iscollection>0</D:iscollection><D:creationdate>" );

	  buf[fmt_iso8601(buf,ss.st_ctime)]=0;
	  array_cats(&x,buf);
	  array_cats(&x,S_ISDIR(ss.st_mode) ?
		     "</D:creationdate><D:resourcetype><D:collection/></D:resourcetype></D:prop></D:propstat></D:response>" :
		     "</D:creationdate><D:resourcetype/></D:prop></D:propstat></D:response>");
	  array_cats(&x,"</D:multistatus>");
	  c=h->hdrbuf=malloc(200+array_bytes(&x));
	  if (!c) goto oom;
	  c+=fmt_str(c,"HTTP/1.1 207 Multi Status\r\nContent-Type: text/xml\r\nServer: " RELEASE "\r\nDate: ");
	  c+=fmt_httpdate(c,now.sec.x-4611686018427387914ULL);
	  c+=fmt_str(c,"\r\nContent-Length: ");
	  c+=fmt_ulong(c,array_bytes(&x));
	  c+=fmt_str(c,"\r\n\r\n");
	  c+=fmt_copybytes(c,array_start(&x),array_bytes(&x));
...
rangekaputt:
		...
		buffer_putnlflush(buffer_1);
	      ...
	      httperror(h,"416 Bad Range","The requested range can not be satisfied.",method==HEAD);
	      goto fini;
	    }
	  }
	}
	...
	if (!c) { oom: httperror(h,"500 Sorry","Out of Memory.",method==HEAD); goto fini; }
	if (ss.st_mtime<=ims) {
	  c+=fmt_str(c,"HTTP/1.1 304 Not Changed");
	  method=HEAD; range_last=range_first;
	  io_close(fd); fd=-1;
	  multirange=0;
	} else {
	...
	if (multirange) {
	  ...
	  c+=fmt_str(c,"\r\nContent-Length: ");
	  if (range_last==ss.st_size && range_last>0) range_last-=(add=1);
	  c+=fmt_ulonglong(c,range_last-range_first+add);
	}
	...
	if (method!=HEAD && (range_first || range_last!=ss.st_size)) {
	  c+=fmt_str(c,"\r\nContent-Range: bytes ");
	...
	if (range_first>ss.st_size) {
	  ...
	  if (method!=HEAD) {
		...
		x=c;
	      ...
	      iob_addfile_close(&h->iob,fd,range_first,range_last-range_first+(range_last>range_first));
	  } else
	    if (fd!=-1) io_close(fd);
	  if (logging) {
	    if (h->hdrbuf[9]=='3') {
	      buffer_putm(buffer_1,methods[method],"/304 ");
	    } else {
	      buffer_putm(buffer_1,methods[method]," ");
	    }
	  }
...
#ifdef SUPPORT_BROTLI
	  case BROTLI: buffer_puts(buffer_1,".br"); break;
#endif
#ifdef SUPPORT_BZIP2
...
kaputt:
  ...
  if (h->still_to_copy) {
	...
	while (l) {	/* this basically can't happen */
	  ...
	  tmp[4]=chunk>>8;
	  tmp[5]=chunk&0xff;
	  if (!iob_addbuf_free(&H->iob,tmp,8) ||
	...
	if (!iob_addbuf(&H->iob,array_start(&H->r),l))
	  goto kaputt;
	H->r.initialized=0;
      }
    }
...
//  printf("CGI: reqlen %u\n",reqlen);
  ...
  if (reqlen==0) { /* SSH MODE */
    ...
    r=vfork();
#endif
    if (r==-1) { close(s); return; }	/* nothing we can do */
    if (r==0) { /* child */
 ...
 * environment, e.g. Host: foo:80 -> HTTP_HOST=foo:80 */
			...
			*file=0;
			...
			cginame=file;
		      ...
		      /* attempt to defend against CVE-2014-6271 */
		      {
			char* c;
			size_t i;
			for (i=0; (c=envp[i]); ++i) {
